        ...
        public async Task<ActionResult> RegisterPatient(Patient patient)
        {
            if (ModelState.IsValid)
            {
                ...
                if (check == null)
                {
                    ...
                    await _context.Entities.AddAsync(new Entity(patient.Id, "AspNetUsers", patient,patient.GetController()));
        ...
        public async Task<ActionResult> RegisterSecretary(Secretary secretary)
        {
            if (ModelState.IsValid)
            {
                ...
                if (check == null)
                {
                    ...
                    await _context.Entities.AddAsync(new Entity(secretary.Id, "AspNetUsers", secretary,secretary.GetController()));
                    await _context.SaveChangesAsync();
        ...
        public async Task<ActionResult> RegisterCaregiver(Caregiver caregiver)
        {
            if (ModelState.IsValid)
            {
                ...
                if (check == null)
                {
                    ...
                    await _context.Entities.AddAsync(new Entity(caregiver.Id, "AspNetUsers", caregiver,caregiver.GetController()));
                    await _context.SaveChangesAsync();
